 ...
 *
 */
@Component(value = "orderListBySupplierResponseConverter")
public class OrderListBySupplierResponseConverter implements ObjectConverter<MerchListEntity, Void, OrderListResponse> {
	...
	public MerchResponse convertToMerchEntity(MerchEntity merch, List<OrderStrategyEntity> orderStrategyEntitys, String order_id) {
		MerchResponse merchEntity = new MerchResponse();
		merchEntity.setOrderId(order_id);
		merchEntity.setMerchId(merch.getMerch_id());
		merchEntity.setMerchState(merch.getMerch_state());
		merchEntity.setProductId(merch.getProduct_id());
		merchEntity.setMerchName(merch.getMerch_name());
		...
		//将价格信息和渠道信息放入商品中
		for (OrderStrategyEntity orderStrategyEntity : orderStrategyEntitys) {
			if (order_id.equals(orderStrategyEntity.getOrderId()) && merch.getMerch_id().equals(orderStrategyEntity.getMerchId())) {
				if (orderStrategyEntity.getPrice() != null) {
					merchEntity.setPrice(orderStrategyEntity.getPrice().doubleValue());//商品单价
				}
				if (orderStrategyEntity.getSettlement_price() != null) {
